...
DataOps/MLOps project: **next-day regression of a modeled wheat water-stress
...
An optional DataOps pathway adds dlt ingestion, DuckDB storage, dbt transformations
and contracts, and Dagster orchestration through the same Python feature pipeline.
...
## Latest update — 17 September 2026

Today's DataOps implementation is on `feature/dataops-pipeline`, at commit
`0f3bc50` (following the ingestion commit `5583615`). Today's updates cover:

- Verified offline **dlt → DuckDB → dbt → Python data contract** processing of
  10,407 NASA observations, with source hashes and exact canonical-data checks.
- Two dbt models, enforced schema contracts and **35 SQL data tests**; Dagster
  exposes six executable assets and separate demonstration/training jobs.
- A separate `.venv-dataops`, isolated generated state, Make commands and an
  additional **offline DataOps GitHub Actions workflow**.
- A French academic report: **38 pages, 13 figures and 28 tables**, with an
  editable Word version and PDF (see [Academic report](#academic-report)).

Both workflows succeeded for `0f3bc50`; links and recorded local checks are in
[Verification status](#verification-status--17-september-2026). The bundled production model
remains `v4-2771746c21f7`. DataOps training and evaluation use their own storage
and do not promote a model or run when the application starts.

For the shortest installed-environment walkthrough: `make dataops-demo`, then
`make dataops-inspect`, then `make dagster`. See
[DataOps Pipeline](#dataops-pipeline) for installation and the full demo.
...
├── configs/dagster.yaml             # Local persistent Dagster instance / run queue
├── dbt_project/                     # DuckDB profile, staging/mart models, SQL tests
├── data/{raw,interim,processed}/     # Immutable JSON → validated CSV → features
├── src/
│   ├── data/                        # Collection, preprocessing, validation
│   ├── dataops/                     # dlt, contract, dbt wrapper, Dagster, ML bridge
...
│   └── dataops/                     # Optional integration/provenance/safety tests
...
│   └── dataops/                     # Ignored DuckDB, exports, isolated ML/tool state
├── monitoring/                      # Drift script, HTML/JSON reports, API events
├── ui/                              # Streamlit HTTP client and NASA request builder
├── docs/                            # Methodology, demo, DataOps/deployment evidence
├── rapports/                        # Locally generated French DOCX/PDF and sources
├── .github/workflows/{ci,dataops}.yml # Core quality and optional DataOps CI
...
runs offline tests and the Dagster demo, and uploads dbt/quality evidence. Although
DataOps is optional for local application use, its CI workflow runs on every push
and pull request, and can also be launched manually. The original CI workflow is
unchanged. CI never requests live NASA data or trains a model.
...
to connect local `make ui` to the default Compose API host port, use
`CWS_API_URL=http://localhost:18000 make ui`. The standalone `docker run` example
below instead publishes the API on host port 8000.
...
Both GitHub Actions workflows use Python 3.12 and run on `push`, `pull_request`
and manual `workflow_dispatch`:

| Workflow | Checks | Outputs |
|---|---|---|
| [Python quality and tests](.github/workflows/ci.yml) | Locked application dependencies; Ruff lint/format; pytest; committed model/offline snapshot verification; FastAPI import | Job logs and check status |
| [Optional DataOps pipeline](.github/workflows/dataops.yml) | Separate application/DataOps environments; optional integration tests; offline Dagster demo through features; release verification; dbt docs generation | `dataops-evidence` artifact: ingestion/quality JSON and dbt manifest, run results and catalog |

The DataOps artifact upload uses `if: always()` so available evidence is collected
even after an earlier step fails. These workflows implement **CI only**: neither
builds Docker images, trains/promotes a model, nor deploys to Komodo. Docker and
Komodo deployment remain explicit operations. Successful runs for today's commit
are linked in [Verification status](#verification-status--17-september-2026).
...
The DataOps implementation verification recorded the following results on Python
3.12. These are saved results, not a claim that updating this README reran training
or all tests.

| Check | Recorded result |
|---|---|
| Core pytest suite | 70 passed; optional DataOps module skipped in the core environment |
| Separate DataOps pytest suite | 21 passed |
| dbt | 2 models, 35 passing SQL data tests |
| Ruff | Lint and formatting checks passed |
| Data equivalence | Weather and feature CSVs byte-for-byte identical to the original outputs |
| Feature dataset | 4,692 rows: 3,081 train / 537 validation / 1,074 test |
| Dagster | Successful `dataops_demo` and separately requested `train_evaluate_explicit` runs |
| API/UI regression | Same saved prediction; manual and automatic offline UI checks passed |
| Local Docker verification | API, UI and MLflow built and passed health checks |

The GitHub API checked on 17 September returned successful completed runs for
commit `0f3bc509191d5ec178bd94abd7edfc58289e38e5`:

- [Python quality and tests — run 35234911963](https://github.com/Anass-Erf/MLOPS_Project/actions/runs/35234911963)
- [Optional DataOps pipeline — run 35234911964](https://github.com/Anass-Erf/MLOPS_Project/actions/runs/35234911964)

No Pull Requests were returned by that check (all states). The DataOps commits are
published on `feature/dataops-pipeline`; the locally recorded `origin/main` still
points to `85f9ace`. No merge into main or remote Komodo verification is claimed.

See [DataOps verification](docs/dataops-verification.md),
[recorded local evidence](docs/dataops-evidence.json),
[API evidence](artifacts/api_verification.json) and
[the saved example response](artifacts/example_response.json).
The DataOps verification/PR notes and the JSON `publication` block were written
before the final commit and push: their statements that publication and remote CI
are pending are historical and are superseded by the commit and run links above.

## Academic report

The French university report generated on 17 September contains 12 chapters,
annexes A–J, a bibliography, 13 figures and 28 numbered tables. The PDF has
**38 pages**. Word includes a dynamic table of contents, automatic figure/table
lists and seven editable student-name placeholders; use **Ctrl+A, F9** to update
fields after editing.

- [Editable Word report](rapports/Rapport_Academique_MLOps_DataOps_Stress_Hydrique_Final.docx)
- [PDF report](rapports/Rapport_Academique_MLOps_DataOps_Stress_Hydrique_Final.pdf)
- [Chapter, figure and table inventory](rapports/Lisez-moi.md)

These files are currently generated locally in `rapports/` and must be included
when sharing the report; they are not yet part of the published DataOps commit.
The report distinguishes the simulated target from field measurements, identifies
the stronger persistence comparator, and labels the Agile sprint reconstruction
and unverified remote deployment honestly.
...
| Branch | `main` for the existing application release; `feature/dataops-pipeline` includes the 17 September DataOps additions |
